import React, { Component } from "react";
import "./students.css";
import chelo from "./../../data/img/Marcelo.jpeg";

class Studentpop extends Component {
  handleClick = () => {
    this.props.toggle();
  };
  render() {
    return (
      <div className="popup-box">
        <div className="box">
          <span className="close-icon" onClick={this.handleClick}>
            x
          </span>
          <div className="student-content">
            <div className="left-side">
              <div className="top-arrow">
                <img
                  height="75px"
                  src="data:image/svg+xml;base64,"
                  alt="top-arrow"
                />
              </div>
              <div className="student-picture">
                <img
                  className="student-picture-img"
                  height="250px"
                  src={chelo}
                  alt="student-picture"
                />
              </div>
              <div className="bot-arrow">
                <img
                  height="75px"
                  src="data:image/svg+xml;base64,"
                  alt="bot-arrow"
                />
              </div>
            </div>
            <div className="right-side">
              <h3 className="student-name">Marcelo Hurtado</h3>
              <h5 className="student-aka">¿Quién?</h5>
              <div className="student-rol">ROL: Project Manager</div>
              <div className="student-career">CAREER: Bioengineering</div>
              <div className="student-age">AGE: 21</div>
              <div className="student-country">COUNTRY: Perú</div>
              <div className="student-hobbies">HOBBIES: play with ganado</div>
              <div className="student-resumee">
                RESUMEE: Lorem ipsum dolor sit, amet consectetur adipisicing
                elit. Deleniti a in nam tenetur vitae neque aliquid error ab ex
                eius cupiditate hic, velit blanditiis at iste omnis incidunt
                tempora eum.
              </div>
            </div>
          </div>
        </div>
      </div>
    );
  }
}

export default Studentpop;
